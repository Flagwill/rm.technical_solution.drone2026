...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\columnwidth]{figure/4_详细设计/硬件设计/hardware_wiring.pdf}
    \caption{整体硬件框图}        
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\columnwidth]{figure/4_详细设计/硬件设计/超级电容拓扑图.jpg}
    \caption{超级电容拓扑图}        
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\columnwidth]{figure/4_详细设计/硬件设计/Topology_Sucap_Bridging.png}
    \caption{桥接后的拓扑结构}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\columnwidth]{figure/4_详细设计/硬件设计/Sucap_Controller_Memory.png}
    \caption{STM32内存空间分配}
...
\begin{figure}[hbt!]
    ...
    \subfigure[超级电容单体保护电路原理图 \label{fig:sucap_single}]{
        \centering
        \includegraphics[width=0.48\columnwidth]{figure/4_详细设计/硬件设计/Sucap_Single.png}
    }
    \subfigure[充电电流采样电路原理图 \label{fig:charge_current_sample}]{
        \centering
        \includegraphics[width=0.44\columnwidth]{figure/4_详细设计/硬件设计/Charge_Current_Sample.png}
    }
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.8\columnwidth]{figure/4_详细设计/硬件设计/Charge_Circuit_Schematic.png}
    \caption{充电模块原理图}
...
\begin{figure}[hbt!]
    ...
    \subfigure[使用BTS443P作为高侧开关 \label{fig:high_side_switch_pmos}]{
        ...
        \centering
        \includegraphics[width=0.7\columnwidth]{figure/4_详细设计/硬件设计/BTS443P_Switch.png}
        \end{minipage}
    }
    \subfigure[使用BTS443P作为高侧开关 \label{fig:high_side_switch_bts443p}]{
        \begin{minipage}[t]{0.45\linewidth}
        \centering
        \includegraphics[width=0.7\columnwidth]{figure/4_详细设计/硬件设计/HighSideSwitch-LM5060.png}
        \end{minipage}
    }
...
\begin{figure}[hbt!]
    ...
    \subfigure[无浪涌吸收刹车 \label{fig:no_surge_absorbing_brake}]{
        \centering
        \includegraphics[width=0.23\columnwidth]{figure/4_详细设计/硬件设计/No_Surge_Absorbing_Brake.png}
    }
    \subfigure[有浪涌吸收刹车 \label{fig:have_surge_absorbing_brake}]{
        \centering
        \includegraphics[width=0.23\columnwidth]{figure/4_详细设计/硬件设计/Have_Surge_Absorbing_Brake.png}
    }
    \subfigure[有浪涌吸收正反切 \label{fig:no_surge_absorbing_switch}]{
        \centering
        \includegraphics[width=0.23\columnwidth]{figure/4_详细设计/硬件设计/No_Surge_Absorbing_Switch.png}
    }
    \subfigure[无浪涌吸收正反切 \label{fig:have_surge_absorbing_switch}]{
        \centering
        \includegraphics[width=0.23\columnwidth]{figure/4_详细设计/硬件设计/Have_Surge_Absorbing_Switch.png}
    }
...
\begin{figure}[hbt!]
    ...
    \subfigure[早期版本电路 \label{fig:surge_absorbing_early}]{
        \centering
        \includegraphics[width=0.4\columnwidth]{figure/4_详细设计/硬件设计/Surge_Absorbing_Early.png}
    }
    \subfigure[现在版本电路 \label{fig:surge_absorbing_now}]{
        \centering
        \includegraphics[width=0.4\columnwidth]{figure/4_详细设计/硬件设计/Surge_Absorbing_Now.png}
    }
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.6\columnwidth]{figure/4_详细设计/硬件设计/Boost_Version_Compare.jpg}
    \caption{升压1.0版本大小对比}     
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.9\columnwidth]{figure/4_详细设计/硬件设计/四相连接图.png}
    \caption{四相拓扑连接图}        
    \label{fig:四相拓扑连接图}
\end{figure}
LTC3897与LTC3784相比,其在宽输入范围的同时，内置了一个理想二极管控制器和浪涌停止器，三个控制器可以单独使用，这让我们得以节省一个输出端的高侧开关模块。单颗LTC3897实现两相PWM互补输出，两个并联通过芯片内部的锁相环实现频率同步，通过PHASMD引脚悬空实现相为$90^{\circ}$互补，让双片四相的方案得以实现。
 ...
 \begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.7\columnwidth]{figure/4_详细设计/硬件设计/boost.pdf}
    \caption{升压2.0原理图}        
    \label{fig:升压2.0原理图}
\end{figure}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.26\columnwidth ,angle=90]{figure/4_详细设计/硬件设计/Boost2.0_test.jpg}
    \caption{升压2.0测试版}        
    \label{fig:升压2.0测试版}
\end{figure}
...
\begin{figure}[hbt!]
    ...
    \subfigure[$C_{ff}$与$C_{flt}$选择范围 \label{fig:boost_cff_cfft_range}]{
        \centering
        \includegraphics[width=0.44\columnwidth]{figure/4_详细设计/硬件设计/Boost_Cff_Cfft_Range.png}
    }
    \subfigure[环路带宽限制 \label{fig:boost_bandwidth_limit}]{
        \centering
        \includegraphics[width=0.44\columnwidth]{figure/4_详细设计/硬件设计/Boost_Bandwidth_Limit.png}
    }
    \caption{Boost环路补偿计算}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\columnwidth]{figure/4_详细设计/硬件设计/Boost_Loop_Compensation_Sch.PNG}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\columnwidth]{figure/4_详细设计/硬件设计/Boost_Type2_Compensation_Relation.PNG}
...
本模块的设计将使用ADI提供的LTpowerCAD以及LTspice等工具进行辅助设计和仿真。如 \figref{fig:ltpowercad_power_stage} 所示，首先使用LTpowerCAD辅助确定包括环路补偿在内的参数。确定参数后，使用LTspice仿真电源的运行效果。如 \figref {fig:ltspice_simulation} 所示，搭建原理图，设置输入电压为$\qty{13.8}{\V}$，仿真电感纹波电流最大的状况，以确定最大的输出纹波电压。仿真得纹波电压为$\qty{0.031}{\V}$，纹波系数仅有\qty{1.12}{\perthousand}。至此，升压模块设计告一段落。

\begin{figure}[hbt!]
    \centering
    \subfigure[LTpowerCAD功率级设计页面 \label{fig:ltpowercad_power_stage}]{
        \centering
        \includegraphics[width=0.5\columnwidth]{figure/4_详细设计/硬件设计/Boost_LTPowerCAD.png}
    }
    \subfigure[在LTspice中搭建仿真原理图 \label{fig:ltspice_simulation}]{
        \centering
        \includegraphics[width=0.45\columnwidth]{figure/4_详细设计/硬件设计/Boost_LTSPICE.png}
    }
...
\begin{figure}[hbt!]
    ...
    \subfigure[中心板PCB 3D图\label{fig:中心板PCB 3D图} ]{
        \centering
        \includegraphics[height=0.4\columnwidth]{figure/4_详细设计/硬件设计/中心板.png}
    }
    \subfigure[中心板实物图\label{fig:中心板实物}]{
        \centering
        \includegraphics[height=0.4\columnwidth]{figure/4_详细设计/硬件设计/中心板实物.jpg}
    }
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.9\columnwidth]{figure/4_详细设计/硬件设计/nucpower.png}
    \caption{LM3150原理图}        
...
\begin{figure}[hbt!]
    ...
    \subfigure[BMI088原理图\label{fig:BMI088_设计方案} ]{
        \centering
        \includegraphics[height=0.4\linewidth]{figure/4_详细设计/硬件设计/BMI088.png}
    }
    
    \subfigure[IMUbase原理图\label{fig:IMUbase_设计方案}]{
        \centering
        \includegraphics[height=0.4\linewidth]{figure/4_详细设计/硬件设计/IMUbase.png}
    }
...
\begin{figure}[hbt!]
    ...
    \subfigure[紫外光LED灯板原理图\label{fig:紫外光LED灯板_设计方案} ]{
        \centering
        \includegraphics[height=0.3\columnwidth]{figure/4_详细设计/硬件设计/紫外光LED灯板.png}
    }
    \subfigure[LED恒流驱动板原理图\label{fig:LED恒流驱动板_设计方案}]{
        \centering
        \includegraphics[height=0.3\columnwidth]{figure/4_详细设计/硬件设计/LED恒流驱动板.png}
    }
