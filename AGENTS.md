...
tex/                  正文：每章一个文件夹，命名与 PDF 章节一致
  0_前言  1_绪论  2_竞品与需求分析  3_概念设计  4_详细设计  5_总结与展望  附录
  tex/4_详细设计/4_详细设计.tex 为第 4 章主文件，内部 \input 聚合各设计小节
  tex/技术方案复盘.tex、tex/表格模板.tex 为参考/模板文件，不属于正文章节

figure/               图片，子目录与 tex 章节对应
  封面  4_详细设计/{机械结构设计,硬件设计,软件设计}  5_总结与展望
```
...
- 表格写法可参考 `tex/表格模板.tex`（含 `\multirow`、`\makecell`、`\longtable` 用法）。
